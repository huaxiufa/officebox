    ...
    public ResponseEntity<String> pdf(@RequestParam("file") MultipartFile file,
        ...
        try(PDDocument doc=PDDocument.load(file.getBytes())) {
            ...
            for(int i=0;i<pages;i++) {
                Path image=dir.resolve("page-"+i+".png"), out=dir.resolve("page-"+i+".txt");
                renderer.renderImageWithDPI(i,150).write(image.toFile());
                Process p=new ProcessBuilder("tesseract",image.toString(),out.toString().replace(".txt",""),"-l",safeLang(lang),"--psm","3").redirectErrorStream(true).start();
                if(!p.waitFor(60,TimeUnit.SECONDS)){p.destroyForcibly();continue;}
                if(p.exitValue()==0&&Files.exists(out)) { text.append("\n--- 第 ").append(i+1).append(" 页 ---\n").append(Files.readString(out)); }
            }
            return ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(text.toString().trim());
