...
public class PdfLayoutAnalyzer {
  public List<PageBlock> analyze(List<PageBlock> input) {
    ...
    for (PageBlock block : blocks) {
      if (!(block instanceof TextBlock tb) || !consumed.contains(tb)) result.add(block);
    }
  ...
  private TableMatch detectTable(List<TextBlock> text) {
    ...
    for (TextBlock block : text) {
      ...
      if (row == null || Math.abs(row.getFirst().bounds().y() - block.bounds().y())
          > Math.max(4, block.bounds().height() * .65)) {
        row = new ArrayList<>();
        rows.add(row);
      }
      row.add(block);
    ...
    rows = rows.stream()
        .map(r -> r.stream().sorted(Comparator.comparingDouble(b -> b.bounds().x())).toList())
        .toList();
    int columns = rows.stream().mapToInt(List::size).min().orElse(0);
    if (columns < 2 || columns > 8) return null;
    final int detectedColumns = columns;
    long strongRows = rows.stream()
        .filter(r -> r.size() == detectedColumns || r.size() == detectedColumns + 1)
        .count();
    if (strongRows < Math.max(3, rows.size() * .65)) return null;
    List<TextBlock> candidateCells = rows.stream().flatMap(List::stream).toList();
    double averageLength = candidateCells.stream()
        .mapToInt(b -> b.text().trim().length()).average().orElse(999);
    if (detectedColumns == 2 && averageLength > 36) return null;
    double xSpread = columnSpread(rows, detectedColumns);
    if (xSpread < 12) return null;
    List<List<TextBlock>> normalized = rows.stream()
        .map(r -> r.size() <= detectedColumns ? r : r.subList(0, detectedColumns))
        .toList();
    ...
    return new TableMatch(
        new TableBlock(new BoundingBox(minX, minY, maxX - minX, maxY - minY), normalized), cells);
  }
  ...
  private double columnSpread(List<List<TextBlock>> rows, int columns) {
    ...
    for (int c = 1; c < columns; c++) {
      ...
      List<Double> xs = rows.stream().filter(r -> r.size() >= columns)
          .map(r -> r.get(columnIndex).bounds().x()).toList();
  ...
  private static Comparator<TextBlock> topDown() {
    return Comparator.comparingDouble((TextBlock b) -> b.bounds().y())
