...
import { ref } from 'vue'
import { FileText, Upload, Download, Loader2, X, ShieldCheck, Scissors, Minimize2 } from 'lucide-vue-next'
...
<template>
  <div class="pdf">
    <div class="badge">PDF WORKSPACE · v2</div>
    <div class="head"><div class="title-icon"><FileText /></div><div><h1>PDF 工具箱</h1><p>合并 · 拆分 · 压缩 · 旋转 · 加密 · 解密</p></div></div>
    ...
    <div class="tools"><button :disabled="busy" @click="merge">合并 PDF</button><button :disabled="busy || files.length !== 1" @click="split">拆分 PDF</button><button :disabled="busy || files.length !== 1" @click="compress">压缩 PDF</button><button :disabled="busy || files.length !== 1" @click="rotate">旋转 90°</button><button :disabled="busy || files.length !== 1" @click="encrypt">加密 PDF</button><button :disabled="busy || files.length !== 1" @click="decrypt">解密 PDF</button></div>
    <div class="hint">🔐 解密必须知道原密码，系统不会猜测或破解密码。压缩采用 Ghostscript 实际重编码图片内容；如果压缩后反而更大，会自动保留原文件。</div>
