...
const file = ref<File | null>(null), taskId = ref(''), status = ref(''), progress = ref(0), message = ref(''), error = ref('')
...
function setFile(f: File) { file.value = f; error.value = ''; status.value = ''; progress.value = 0 }
function choose(e: Event) { const f = (e.target as HTMLInputElement).files?.[0]; if (f) setFile(f) }
function drop(e: DragEvent) { const f = e.dataTransfer?.files?.[0]; if (f) setFile(f) }
function applyTask(task: any) { status.value = task.status || ''; progress.value = task.progress ?? 0; message.value = task.progressMessage || ''; if (status.value === 'FAILED') error.value = task.error || '转换失败' }
async function convert() {
  ...
  try {
    ...
    const body = await response.json(); const task = body.data ?? body
    taskId.value = task.id; applyTask(task); poll()
...
function poll() {
  ...
  const run = async () => {
    try {
      ...
      const body = await response.json(); applyTask(body.data ?? body)
      if (['SUCCESS', 'FAILED'].includes(status.value) && timer) window.clearInterval(timer)
    } catch (e) { if (timer) window.clearInterval(timer); error.value = e instanceof Error ? e.message : '读取任务状态失败' }
  }
  run(); timer = window.setInterval(run, 1000)
}
</script>
<template>
  <div class="tool"><div class="tool-head"><div><h2>PDF → Word</h2><p>将 PDF 转换为可编辑的 DOCX 文档</p></div><FileText :size="28"/></div>
    <label class="drop" @dragover.prevent @drop.prevent="drop"><Upload :size="28"/><b>{{ file ? file.name : '拖拽 PDF 到这里' }}</b><span>{{ file ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : '或点击选择 PDF 文件' }}</span><input type="file" accept="application/pdf,.pdf" @change="choose" hidden /></label>
    ...
    <div v-if="status" class="state"><div class="state-line"><span>{{ message }}</span><b>{{ progress }}%</b></div><div class="bar"><i :style="{ width: `${progress}%` }"></i></div><div v-if="status === 'SUCCESS'" class="success"><CircleCheck :size="18"/> 转换完成 · <a :href="resultUrl">下载 DOCX</a><Download :size="16"/></div><div v-else-if="status === 'FAILED'" class="failure"><CircleX :size="18"/> {{ error || '转换失败' }}</div></div><p v-if="error && status !== 'FAILED'" class="failure">{{ error }}</p>
  </div>
</template>
<style scoped>
